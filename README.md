...
This module will pull JSON formatted data from web based APIs, or local files previously loaded. It supports standard OUATH2 authentication for web based APIs. Thedata is then formatted into NDTF format, that simplifies the data to the lowest level, based on a Subject, an Object within that subject and a value. A timestamp is also included.
